...
本教程面向希望在Windows、macOS操作系统上以极低成本快速搭建AI编程环境的学生用户。根据读者需求，可以选择不同的阅读起点：
...
若要在 Windows 系统中安装 Claude Code，请从第 2 节开始阅读；若要在 macOS 中安装，请从第 3 节开始阅读。

## 2. 使用ZCF安装Claude Code (Windows)<a id="ZCF"></a>
...
## 3. 使用 brew 安装 Claude Code (macOS)<a id="brew"></a>

### 3.1 安装 Homebrew

对于 macOS 用户，系统自带 bash、git 和 curl，仅需再安装 CLT for Xcode 即可。在终端中输入
```bash
xcode-select --install
```

接着，在终端输入以下几行命令设置环境变量，使 Homebrew： 的链接指向清华源，便于国内下载各类包：
```bash
...
其中，第一行是从清华源克隆下载 Homebrew 安装包文件，第二行是执行 .sh 安装文件，第三行为删除安装包。

一般情况下安装的 Homebrew 是最新版本，如果想手动更新版本，可以使用下述命令：
```bash
brew update
```
...
在终端中输入下述命令即可安装 Claude Code
```bash
brew install anthropic/claude/claude
```

安装成功后，在终端中输入```claude -V```查看 Claude Code 的版本号。如果输出版本号，则安装成功。

### 3.3 设置 Claude Code

首次运行 Claude Code 前，其配置文件 ~/.claude/settings.json 不会自动创建。可以在终端中输入```claude```运行 Claude Code，按照提示完成初始设置。退出后即可在路径 ~/.claude 下看到 settings.json 配置文件。

若要配置 API，可以打开该 settings.json 文件，在其中插入：
```json
"env": {
...
具体配置信息可以查看 2.5 节内容，根据对应模型供应商提供的示例进行配置。

## 4. 在IDE中集成Claude Code <a id="IDE"></a>
...
<div style="margin: 0 auto; text-align: center; width: 88%"><img src="figures/code_in_integrated_terminal.webp" />在集成终端中使用Claude Code</div>
...
<div style="margin: 0 auto; text-align: center; width: 88%"><img src="figures/code_in_panel.webp" />利用插件调用Claude Code</div>
